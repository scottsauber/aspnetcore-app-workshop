...
1. Add a new `Speaker` class using the following code:
    ...
    ```
1. Next we'll create a new Entity Framework DbContext.  

2. Install the following packages:
- Microsoft.EntityFrameworkCore
- Microsoft.EntityFrameworkCore.Tools
- If on Windows: Microsoft.EntityFrameworkCore.SqlServer.  
- If on Mac: Microsoft.EntityFrameworkCore.Sqlite
**If you're not using Visual Studio** install the package from the command line with `dotnet add package <PackageNameGoesHereWithoutTheAngleBrackets>`

3. Create a new `ApplicationDbContext` class in the `Models` folder using the following code:
    ```csharp
    ...
    ```
4. Add a connection string to the appsettings.json file for this database:
...
1. Add a reference to the NuGet package `Microsoft.EntityFrameworkCore.Sqlite`. Note that you'll need to resolve several references by adding missing `using` statements (`Ctrl + .` is your friend).
    ...
    > **If you're not using Visual Studio** install the package from the command line with `dotnet add package Microsoft.EntityFrameworkCore.Sqlite`
...
1. Run the following commands in the Package Manager Console
   ...
   dotnet ef migrations add Initial --context ApplicationDbContext --project .\BackEnd\BackEnd.csproj
   dotnet ef database update --context ApplicationDbContext --project .\BackEnd\BackEnd.csproj
   ```
...
## A quick look at the WeatherForecastController
First, open the `Controllers` folder and take a quick look at the `WeatherForecastController`. You'll see simple functions that correspond to HTTP verbs. You'll see the output of this controller in a bit, but first we'll build our own API controller for the `Speakers` model class.
...
1. Add the `Swashbuckle.AspNetCore` NuGet package version `5.0.0-rc3` (note: Check the box for "Include prerelease")
   > This can be done from the command line using `dotnet add package Swashbuckle.AspNetCore --version 5.0.0-rc3`
