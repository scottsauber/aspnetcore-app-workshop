...
1. Add a new `Speaker` class using the following code:
    ...
    ```
1. Next we'll create a new Entity Framework DbContext.  Install the following packages:
...
2. Create a new `ApplicationDbContext` class in the `Models` folder using the following code:
    ```csharp
