...
1. Add a new `Speaker` class using the following code:
    ...
    ```
1. Next we'll create a new Entity Framework DbContext.  Install the Microsoft.EntityFrameworkCore package.  If on Windows, install the Microsoft.EntityFrameworkCore.SqlServer package.  If on Mac, install the Microsoft.EntityFrameworkCore.Sqlite package
   > **If you're not using Visual Studio** install the package from the command line with `dotnet add package Microsoft.EntityFrameworkCore` and `dotnet add package Microsoft.EntityFrameworkCore.Sqlite`
