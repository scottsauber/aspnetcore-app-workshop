...
1. If using Visual Studio, right-click Dependencies, go to Manage NuGet Packages, go to Browse, then search for  Microsoft.AspNet.WebApi.Client and install it
2. If using Visual Studio code, open the command prompt and navigate to the same folder as /FrontEnd/Frontend.csproj via `dotnet add package Microsoft.AspNet.WebApi.Client package`
...
1. In this folder, add a new interface called `IApiClient` with the following members:
   ...
   namespace FrontEnd.Services
   {
       public interface IApiClient
       {
          ...
          Task<bool> AddAttendeeAsync(AttendeeDto attendee);
...
1. Staying in this folder, add a new class called `ApiClient` that implements the `IApiClient` interface by using `HttpClient` to call out to our BackEnd API application and JSON serialize/deserialize the payloads:
   ...
   namespace FrontEnd.Services
   {
       public class ApiClient : IApiClient
       {
           ...
           public async Task<bool> AddAttendeeAsync(AttendeeDto attendee)
           {
...
### Add DTOs for search
1. Change the ConferenceDTO project to netcoreapp3.0 so that we can use the new System.Text.Json API's.
1. Right-click the ConferenceDTO project and click Edit Project File
1. Change `netstandard2.0` to `netcoreapp3.0`
       ...
       public class SearchController : ControllerBase
       {
           ...
           public async Task<IActionResult> Search(SearchTerm term)
           {
               ...
               var results = sessionResults.Select(s => new SearchResult
               {
                   ...
                   Value = JsonSerializer.Serialize(s.MapSessionResponse())
               })
               .Concat(speakerResults.Select(s => new SearchResult
               {
                   Type = SearchResultType.Speaker,
                   Value = JsonSerializer.Serialize(s.MapSpeakerResponse())
               }));
      ...
      SearchResults = results.Select(sr =>
      {
          switch (sr.Type)
          {
              case SearchResultType.Session:
                  return JsonSerializer.Deserialize<SessionResponse>(sr.Value);
              case SearchResultType.Speaker:
                  return JsonSerializer.Deserialize<SpeakerResponse>(sr.Value);
              default:
                  return (object)sr;
          }
      })
